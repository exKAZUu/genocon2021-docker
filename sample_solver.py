...
def solve(inf):
    ...
    padded_reads = []
    padded_entire_len = len(padded_reference_sequence)
    for i, line in enumerate(lines):
        ...
        if padded_string.find("=") != -1:
            print("ERROR")
            print(padded_string)
            sys.exit(2)
        padded_reads.append(padded_string + (' ' * (padded_entire_len - len(padded_string))))
        # print("%3d %c: %s" % (i, orientation, padded_string))
    output_base = []
    for i in range(padded_entire_len):
        count = {}
        count[' '] = 0
        count['-'] = 0
        count['A'] = 0
        ...
        count['C'] = 0
        count['G'] = 0
        for r in padded_reads:
            # print(r[i])
            count[r[i]] += 1
        del count[' ']
        max_char = None; max_count = -1
        for k, v in count.items():
            if max_count < v:
                max_char = k
                max_count = v
        if max_char != '-':
            output_base.append(max_char)
    base_consensus = "".join(output_base)
